...
void Pinky::OnHitCollision(GameObjectBase* hit_object)
{
	//当たったオブジェクトが壁だったら
...
eEnemyState Pinky::GetEnemyState() const
{
	return enemy_state;
}

bool Pinky::GetDestroy() const
{
	return is_destroy;
}
...
void Pinky::Movement(float delta_second)
{
	//移動量から移動方向を更新
	...
	default:
		velocity = 0.0f;
		now_direction_state = next_direction_state;
		next_direction_state = Pinky::LEFT;
		break;
	}
